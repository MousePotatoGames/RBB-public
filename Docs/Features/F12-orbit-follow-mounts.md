...
**Verified (automated)** — 2026-08-07.
EditMode 174/174, PlayMode 106/106, 새 콘솔 에러·경고 0건.
검증 보고서: [2026-08-07_F12-orbit-follow-mounts_VERIFICATION.md](../Reports/2026-08-07_F12-orbit-follow-mounts_VERIFICATION.md)
스펙 승인: 2026-08-06 (WPN-008a 신설 + Decision 0002 개정 포함 승인)

**남은 관문은 사람의 눈입니다** — 떨림이 실제로 사라졌는지, 도끼가 공 회전과 무관하게
도는 게 읽히는지는 계기가 판정할 수 없습니다. 보고서의 Manual Verification 목록 참조.

> **플레이 중 보고된 결함 수정 (2026-08-06)**: 사용자 보고 *"따라오는게 덜덜덜 떨려보여"*.
> 원인 두 가지 — (1) 보간이 없어 공(매 렌더 프레임 보간) 대비 마운트 무기(초당 50회)가
> 계단식으로 보임, (2) 안착 후 부동소수점 노이즈 방향으로 매 스텝 회전을 다시 계산.
> `FixedUpdate` 판정 포즈 + `Update` 렌더 보간으로 (1)을, 유효 이동 임계값 + 회전 속도
> 상한으로 (2)를 해결. 회귀 테스트 2개 추가. Open questions의 "저프레임 각짐" 항목은
> **예측이 틀렸던 것**으로 판명되어 아래에 정정을 남깁니다.

> **검증 중 발견한 결함 수정 (2026-08-07)**: 추종 무기가 **월드 원점에서 생성**되어,
> 아레나 외곽에서 캡슐을 먹으면 펫이 맵 중앙에서 날아왔습니다. `Advance(0f)`가 궤도는
> 원 위로 스냅시키지만 추종은 `deltaTime = 0`이면 제자리를 반환하는데, 그 제자리가
> `CreatePrimitive`의 원점이었습니다 — 설계된 위치가 아니라 **생성 순서의 부산물**.
> 부착 방향으로 유지 거리만큼 떨어진 지점에 미리 놓도록 고쳤습니다 (WPN-001a를 추종에도 적용).
> **PlayMode 리그가 공을 항상 원점에 두기 때문에 결함 위치와 정답 위치가 겹쳐 있었습니다** —
> 공을 옮긴 뒤 부착하는 회귀 테스트를 추가했습니다.
...
| `Wpn007_FollowWeapon_AppearsBesideThePlayer_NotAtWorldOrigin` | B5 — 검증 중 추가. 공을 원점 밖으로 옮긴 뒤 부착 |
...
- **펫이 떨지 않는가** — 2026-08-06 수정의 유일한 판정 기준
- **펫과 펫의 투사체가 구분되는가** — 검증 스크린샷에서 둘 다 비슷한 크기의 회색 구입니다
  (펫 0.32 / 투사체 지름 0.30). 그레이박스 한계일 수 있으나 P07에서 의도적으로 갈라야 합니다
...
- ~~**보간 없는 궤도가 60fps 미만에서 각져 보이는가**~~ — **틀린 예측이었습니다.**
  실제 증상은 프레임률과 무관했고, **60fps에서 더 심했습니다**. 공은 물리 보간으로 매 렌더
  프레임 그려지는데 마운트 무기는 초당 50회만 움직여, 공 대비 상대적으로 떨려 보입니다.
  직접 보간으로 해결했습니다 (Rigidbody 없이 15줄).
  → **교훈**: 같은 원인이라도 증상을 잘못 예측하면 우선순위를 잘못 매깁니다.
  저프레임 문제였다면 나중에 봐도 됐지만, 실제로는 첫 플레이에서 바로 보이는 결함이었습니다
