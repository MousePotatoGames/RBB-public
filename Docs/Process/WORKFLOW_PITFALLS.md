...
| B5 | **프레임 수로 기다려** 시간 기반 운동이 미완료 | 2건 | 중 | ✅ unity-testing 규칙 |
| B6 | 물리 콜백 안에서 **`DestroyImmediate`** → 런타임 에러 | 1회 | 높음 | ✅ 풀 생성 규칙 |
...
### B5. 프레임 수로 기다려 시간 기반 운동이 미완료 (중, 2건)

**상황 (F13)**
경험치 오브가 2m 거리를 초당 9m로 끌려온다. 테스트에서 `yield return null`을 **60번** 돌린 뒤
"흡수됐는가"를 물었는데 실패했다. 60프레임이면 1초는 되겠거니 했다.

단언 메시지에 상태를 실어 다시 돌렸더니 답이 나왔다:

```text
[timeScale=1 dt=0.0023 player=(0,0.5,0) orb=(0,0.5,0.64) chasing=True]
```

**`dt = 2.3ms`.** 테스트 러너는 렌더링을 안 하고 프레임 상한도 없어서 **초당 400프레임 넘게** 돈다.
60프레임은 **0.14초**였다. 오브는 정상적으로 쫓아오는 중이었고, 시간이 모자랐을 뿐이다.

**교훈**
프레임 수는 **"일어나지 않는다"를 확인할 때만** 쓸 수 있다. 초당 몇 미터로 움직이는 것을
기다릴 때는 **초로 기다려야 한다.** 프레임률을 가정하는 순간 그 가정은 러너에서 깨진다.

> 같은 실수를 코드 쪽 결함으로 오진할 뻔했다. **단언 메시지에 상태를 실어 다시 돌린 것**이
> 5분 만에 답을 줬다 — 실패한 단언은 "무엇이 틀렸다"만 말하고 "무엇이었다"는 말해주지 않는다.

**플러그인 예방책 (P12)**
`references/unity-testing.md`에:

> 시간 기반 운동을 기다릴 때 `yield return null`을 N번 세지 말 것.
> 테스트 러너는 프레임 상한이 없어 한 프레임이 2~3ms다 — 60프레임은 1초가 아니라 0.15초다.
> `while (Time.time < until) yield return null;`로 **초를 기다릴 것**.
> 프레임 수는 "이 시간 동안 아무 일도 없었다"를 볼 때만 쓴다.
>
> 실패한 단언의 메시지에는 **판정에 쓴 상태를 함께 넣을 것** (timeScale, deltaTime, 좌표).
> 재현 없이 원인이 나온다.

---

### B6. 물리 콜백 안에서 DestroyImmediate (높음, 1회)

**상황 (F13)**
기존 풀 3개(무기·투사체)가 전부 `CreatePrimitive` → `DestroyImmediate(collider)` 패턴을 쓴다.
콜라이더가 한 물리 스텝이라도 살아있으면 공을 밀기 때문에 의도적으로 즉시 파괴한다.

경험치 오브 풀에 그대로 복사했더니 테스트가 이렇게 죽었다:
...
**차이는 호출 지점이었다.** 투사체는 `FixedUpdate`에서 생성된다. 오브는 **처치 이벤트**에서
생성되는데, 그건 `OnCollisionStay` 안이다. 같은 패턴이 한쪽에선 정답이고 한쪽에선 금지다.

**교훈**
패턴을 복사할 때 **패턴이 성립하는 조건까지 복사됐는지** 봐야 한다.
`DestroyImmediate`의 조건은 "물리 콜백 밖에서 불린다"였고, 그건 코드에 안 적혀 있었다.

근본 해결은 **콜라이더를 애초에 만들지 않는 것**이었다 — 프리미티브에서 메시만 한 번 꺼내
캐시하고, 이후엔 `new GameObject` + MeshFilter/MeshRenderer로 만든다. 파괴할 게 없으면
파괴 시점 문제도 없다.

**플러그인 예방책 (P13)**
`references/unity-testing.md`에:

> 풀 객체를 **물리 콜백에서 생성할 수 있다면** `CreatePrimitive` + `DestroyImmediate`를 쓰지 말 것.
> 메시/머티리얼을 한 번 캐시하고 빈 GameObject에 얹을 것 — 콜라이더가 존재한 적이 없으면
> 파괴 시점도 문제가 되지 않는다.
