...
굴러가는 공에 무기가 **실제로 붙고**, 구르면서 그 무기가 작동하는 3D 물리 액션 로그라이트.
공격 버튼이 없다 — 이동·회전·충돌이 곧 공격이다.
...
| | |
|---|---|
| 설계 (단일 진실) | [Docs/Game/](Docs/Game/) — 규칙 ID가 구현·테스트의 기준 |
...
로컬 확인 (Unity WebGL은 gzip 헤더가 필요하다):
...
게임 코드는 이 저장소의 라이선스를 따른다. 아래 서드파티 에셋은 **전부 CC0 1.0 Universal**
(Public Domain Dedication)로, 각 배포 페이지에서 직접 확인했다 (2026-08-01).

> CC0는 **저작자 표시를 요구하지 않는다.** 아래 표기는 의무가 아니라 관례이며,
> 잼·공모전이 사용 에셋 목록을 요구하는 경우에 대비한 기록이다.
...
**미도입** = 라이선스만 확인했고 아직 프로젝트에 넣지 않았다. 현재 빌드는 전부 그레이박스 프리미티브다.
도입 시 `Assets/ThirdParty/<제작자>/` 아래에 원본 그대로 두고, 이 표의 상태를 갱신할 것.

### 새 에셋을 추가할 때

itch.io의 [CC0 태그 목록](https://itch.io/game-assets/new-and-popular/assets-cc0/free/tag-low-poly)은
**업로더가 스스로 붙인 태그**이지 itch.io가 검증한 것이 아니다.
목록에서 골랐다는 이유로 CC0라고 단정하지 말고, **에셋 페이지의 라이선스 표기를 개별로 확인**한 뒤 위 표에 추가할 것.
